...
import java.lang.foreign.*; // Foreign API ~ JNI 2.0
...
public class CpuAffinityWindows {
	public static void main(String[] args) throws Throwable {
		try (Arena arena = Arena.ofConfined()) {
			...
			MethodHandle getCurrentThread = linker.downcallHandle(kernel32.find("GetCurrentThread").orElseThrow(),
					FunctionDescriptor.of(ValueLayout.ADDRESS));

			// DWORD_PTR SetThreadAffinityMask(HANDLE hThread, DWORD_PTR
			// dwThreadAffinityMask)
			MethodHandle setThreadAffinityMask = linker
					.downcallHandle(kernel32.find("SetThreadAffinityMask").orElseThrow(),
